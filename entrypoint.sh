...
for custom_node_directory in /app/custom_nodes/*; do
  if [ -f "$custom_node_directory/requirements.txt" ]; then
    ...
    python -m pip install --root-user-action=ignore --break-system-packages --requirement "$custom_node_directory/requirements.txt" 2> >(while read line; do echo -e "\e[31m$line\e[0m"; done)
  fi
done
