...
for custom_node_directory in /app/custom_nodes/*; do
  if [ -f "$custom_node_directory/requirements.txt" ]; then
    ...
    /opt/venv/bin/python -m pip install --root-user-action=ignore --requirement "$custom_node_directory/requirements.txt" 2> >(while read line; do echo -e "\e[31m$line\e[0m"; done)
  fi
done

## RUN CONTAINER
launch_cmd="/opt/venv/bin/python main.py --listen 0.0.0.0 --port 8188 --disable-auto-launch --enable-manager --reserve-vram 4.0 --disable-cuda-malloc --use-pytorch-cross-attention"
